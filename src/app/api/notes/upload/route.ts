...
export async function POST(req: NextRequest) {
  ...
  // The SDK authenticates via BLOB_READ_WRITE_TOKEN or, when absent, OIDC
  // (VERCEL_OIDC_TOKEN + BLOB_STORE_ID — injected automatically on Vercel).
  // Let it try; surface a clear error only if it genuinely can't authenticate.
  try {
    ...
    return NextResponse.json({ error: `Image upload failed: ${message}` }, { status: 503 });
  }
}

...
export function BudgetView({ slug, fields }: { slug: string; fields: SectionFieldDef[] }) {
  ...
  const typeField = fields.find((f) => f.type === "select" && (f.options || []).some((o) => o.toLowerCase() === "income"));
  const labelField = fields.find((f) => f.type === "text");

  const amountKey = amountField?.key ?? "amount";
  const typeKey = typeField?.key ?? "type";
  const isIncome = (e: { data: Record<string, unknown> }) =>
    String(e.data[typeKey]).toLowerCase() === "income";

  let income = 0;
  let expense = 0;
  for (const e of entries) {
    const amt = Number(e.data[amountKey]) || 0;
    if (isIncome(e)) income += amt;
    else expense += amt;
  }
            ...
            <div className="space-y-1">
              {entries.map((e) => (
                <div key={e._id} className="flex items-center py-2 border-b" style={{ borderColor: "var(--border-subtle)" }}>
                  <span className="w-20 text-xs text-[var(--text-muted)]">{format(parseISO(String(e.date).slice(0, 10)), "MMM d")}</span>
                  <span className="flex-1 text-sm text-[var(--text-primary)]">{labelField ? String(e.data[labelField.key] ?? "") : ""}</span>
                  <span className="num text-sm font-medium" style={{ color: isIncome(e) ? "var(--positive, #2e6b4f)" : "var(--text-primary)" }}>
                    {isIncome(e) ? "+" : "−"}{(Number(e.data[amountKey]) || 0).toFixed(2)}
                  </span>

...
export function DashboardCards({
  ...
  if (has("gym")) {
    statCards.push(<StatCard key="gym" label="Gym this week" value={`${gymDaysThisWeek}/${gymTargetDays}`} icon={<Dumbbell size={16} />} />);
  }
  if (has("study")) {
    statCards.push(<StatCard key="study" label="Study this week" value={`${studyHours}h`} icon={<GraduationCap size={16} />} />);
  }
  if (has("hobbies")) {
    statCards.push(<StatCard key="hobbies" label="Hobby hours" value={`${hobbyHours}h`} icon={<Palette size={16} />} />);
  }
  if (has("housework")) {
    statCards.push(<StatCard key="housework" label="Tasks today" value={houseworkTotal > 0 ? `${houseworkDone}/${houseworkTotal}` : "—"} icon={<Home size={16} />} />);
  }
  if (has("health")) {
    statCards.push(<StatCard key="health" label="Avg sleep" value={avgSleep > 0 ? `${avgSleep.toFixed(1)}h` : "—"} icon={<Heart size={16} />} />);
  }
  if (has("goals")) {
    statCards.push(<StatCard key="goals" label="Active goals" value={String(activeGoalCount)} icon={<Target size={16} />} />);
  }
  if (has("reading")) {
    statCards.push(<StatCard key="reading" label="Reading now" value={String(readingCount)} icon={<BookOpen size={16} />} />);
  }
  if (has("journal")) {
    statCards.push(<StatCard key="journal" label="Entries this month" value={String(journalCount)} icon={<NotebookPen size={16} />} />);
  }
  if (has("shopping")) {
    statCards.push(<StatCard key="shopping" label="Items pending" value={String(pendingItems)} icon={<ShoppingCart size={16} />} />);
  }
  if (has("mealprep")) {
    statCards.push(<StatCard key="mealprep" label="Meals this week" value={String(mealsPlanned)} icon={<UtensilsCrossed size={16} />} />);
  }

  return (
    <div className="space-y-6">
      {statCards.length > 0 && (
        <div className={`grid grid-cols-2 ${GRID_COLS[Math.min(statCards.length, 4)] || "md:grid-cols-4"} gap-3`}>
          {statCards}
        </div>
      )}
...
function StatCard({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) {
  return (
    <Card padding="md" className="glass-lift cursor-default">
      ...
      {work.hourlyRate > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <p className="text-sm font-semibold mt-0.5">{formatCurrency(work.todayEarnings, currency)}</p>
          </div>
          <div>
            <p className="stat-label">Week</p>
            <p className="text-sm font-semibold mt-0.5">{formatCurrency(work.weekEarnings, currency)}</p>
          </div>
          <div>
            <p className="stat-label">Month</p>
            <p className="text-sm font-semibold mt-0.5">{formatCurrency(work.monthEarnings, currency)}</p>
          </div>
        </div>
      )}
    </Card>

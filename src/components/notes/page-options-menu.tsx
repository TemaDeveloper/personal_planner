  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" aria-label="Page options" onClick={() => setOpen((o) => !o)}
        className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-[var(--surface-raised)] transition-colors" style={{ color: "var(--text-muted)" }}>
        <MoreHorizontal size={18} />
      </button>
      {open && (
        <div
          className="absolute right-0 mt-1 w-52 rounded-lg border p-1 z-50 origin-top-right animate-[notesPop_120ms_ease-out]"
          style={{ background: "var(--surface-1)", borderColor: "var(--border-default)", boxShadow: "0 8px 24px rgba(0,0,0,.14)" }}>
          <button type="button" onClick={() => { onToggleFavorite(); setOpen(false); }}
            ...
            <span className="flex items-center gap-2">
              <Star size={15} fill={pinned ? "currentColor" : "none"} style={{ color: pinned ? "var(--accent-color)" : "currentColor" }} />
